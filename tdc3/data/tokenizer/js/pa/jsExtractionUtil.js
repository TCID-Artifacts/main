(function () {
    ...
    function getTokens(code) {
        try {
            const tokenizer = acorn.tokenizer(code, { locations: true, ranges: true, ecmaVersion: ecmaVersion });
            const tokens = [];
    ...
    function getAST(code, noLocations) {
        try {
            if (noLocations) return acorn.parse(code, { ecmaVersion: ecmaVersion });
            else return acorn.parse(code, { locations: true, ranges: true, ecmaVersion: ecmaVersion });
        } catch (e) {
        }
    }
    ...
    function tokenToString(t, omitKind) {
        let result = "";
        if (omitKind == undefined || omitKind === false) {
    ...
    function tokensToStrings(tokens, omitKind) {
        return tokens.map(tokenToString, omitKind);
    }
    ...
    function getCodeOfSubtree(node, allCode) {
        const firstChar = node.range[0];
        const lastChar = node.range[1];
        return allCode.slice(firstChar, lastChar);
    }
    ...
    module.exports.getCodeOfSubtree = getCodeOfSubtree;
